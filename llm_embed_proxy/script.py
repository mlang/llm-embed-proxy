...
class EmbedVisitedPages:
    ...
    def response(self, flow):
        ...
        self.pool.submit(embed,
            flow.request.url, flow.request.headers
        )
...
def cached_content():
    content = cached(collection(), request.form.get('id'))
    if content is None:
...
def embed(url: str, headers: Headers) -> None:
    metadata, content = pure.md(url, headers)
    content = cleanup(content)
    logger.info(f"Embedding {len(content)} characters")
    try:
        collection(create=True).embed(url, content, metadata, store=True)
